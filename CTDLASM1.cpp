...
void findMiddle(Node* head) {
    ...
    if (fast == nullptr && prev != nullptr) {
        cout << "Hai số giữa là: " << prev->data << " và " << slow->data << endl;
    } else {
        cout << "Số giữa là: " << slow->data << endl;
    }
}
...
int main() {
    ...
    if (hasCycle(head)) cout << "Chu trình tồn tại!" << endl;
    else cout << "Không có chu trình." << endl;
    return 0;
}
...
int main() {
    ...
    cout << "Danh sách sau khi loại bỏ phần tử trùng lặp: ";
    printList(head);
...
int main() {
    ...
    if (isPalindrome(head))
        cout << "Danh sách là Palindrome" << endl;
    else
        cout << "Danh sách không phải là Palindrome" << endl;
...
void searchValue(Node* head, int x) {
    ...
    while (temp != nullptr) {
        if (temp->data == x) {
            found = true;
            cout << "Node với giá trị " << x << " có địa chỉ: " << temp << endl;
        }
        temp = temp->next;
    }

    if (!found) {
        cout << "Không tìm thấy giá trị " << x << " trong danh sách." << endl;
    }
}
